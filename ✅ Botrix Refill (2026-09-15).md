...
## Estado
Funcional: sí | Beta: no (`v1.2.1`) | Última revisión: revisión completa del código y del pipeline de release a pedido del usuario (15/09/2026) — sin errores críticos ni funcionalidad rota. Se corrigió un mensaje engañoso en el Setup ("datos encriptados" cuando en realidad se guardan sin cifrar), se agregó cache de NuGet al workflow de release para publicar versiones más rápido, y se eliminó código/estilos/entradas de build que ya no se usaban. `dotnet build` verificado sin errores después de cada cambio.
