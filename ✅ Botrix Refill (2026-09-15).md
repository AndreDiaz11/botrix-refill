...
## Estado
Funcional: sí | Beta: no (`v1.5.1`) | Última revisión (15/09/2026): tras probar v1.5.0, el usuario reportó 3 cosas más — el emoji 🔔 puesto como reemplazo de emergencia no combinaba con la paleta (reemplazado por un ícono de trazo azul consistente con el resto), el botón "?" de ayuda seguía desalineado (ajustado), y la tabla de recompensas dejaba demasiado espacio vacío entre el nombre y los puntos en ventanas anchas (columna del nombre pasó de ancho flexible a fijo). **Pendiente de confirmación visual del usuario**.
