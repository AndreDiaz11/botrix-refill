...
## Estado
Funcional: sí | Beta: no (`v1.3.0`) | Última revisión: 4 fixes reportados por el usuario tras usar la app en real (15/09/2026) — puntos que no cargaban (causa real: Session-kid inválido/vencido, ahora se avisa en vez de quedar en blanco), botón de canje siempre activo, instancias duplicadas + popup de actualización repetido (faltaba candado de instancia única, ya agregado), rediseño del header del Shop (pendiente de confirmación visual del usuario, no se pudo verificar con captura de pantalla real). `dotnet build` verificado sin errores.
...
## Versión
1.3.0 — fix de puntos que no cargaban, botón de canje siempre activo, instancia única de la app (arregla popup de actualización repetido), rediseño del header del Shop.

## Cambios
1. (15/09/2026) 4 fixes a pedido del usuario tras usar la app en real:
