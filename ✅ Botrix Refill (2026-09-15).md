...
## Estado
Funcional: sí | Beta: no (`v1.5.0`) | Última revisión (15/09/2026): nueva ronda de limpieza visual a pedido del usuario — sacadas las tarjetas de estadísticas (recompensas disponibles/reclamadas) y todo su código de soporte, el streamer repetido en la barra de título, el banner "se actualiza automáticamente", las píldoras verdes/grises del pie, y un ícono de campana dibujado a mano que probablemente renderizaba mal (reemplazado por emoji). Header reordenado: nombre+puntos juntos a la izquierda, hora+Detener juntos a la derecha. **Pendiente de confirmación visual del usuario** (usuario avisó que probará y pedirá más ajustes si hace falta).
