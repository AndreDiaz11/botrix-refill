...
2. **Shop**: header con gradiente azul — avatar/nombre/puntos + streamer, botón ⏹ Detener fijo arriba a la derecha (nunca se superpone, vive en su propia fila del header), y debajo 3 tarjetas con ícono: 🎁 Recompensas disponibles, ⚡ Recompensas reclamadas hoy (contador persistente por día, `Services/RedemptionTracker.cs`), 🕐 Última revisión. Debajo, un aviso informativo fijo ("Canjea tus puntos por recompensas en directo...").
3. **Recompensas agrupadas por tipo, en tabla** (no tarjetas): "Yape / Plin", "Suscripciones", "Recargas", "Otros" — cada grupo con ícono de color propio + una tabla (Recompensa / Puntos / Disponibilidad / Acción). Acción es un botón azul "🎁 Canjear" (copia `!code` al portapapeles y suma al contador de "reclamadas hoy") cuando hay stock, o una píldora gris deshabilitada "🔒 Sin stock" cuando no.
...
## Estado
Funcional: sí | Beta: no (`v1.4.1`) | Última revisión (15/09/2026): usuario confirmó que header en una línea, tope de ancho, fix de scroll y ayuda de Session-kid de v1.4.0 quedaron bien. Pendiente: instancia única — seguía duplicándose al reabrir desde el ícono de la barra de tareas pese al candado de v1.3.0/v1.4.0. Reforzado en v1.4.1 (`Global\` en el nombre del Mutex/Event, manejo de errores más amplio, log de diagnóstico) sin poder reproducir el bug en este entorno — **falta que el usuario confirme si con esto se resuelve**, y si no, revisar `error-log.txt` con las líneas nuevas de diagnóstico. `dotnet build` verificado sin errores.
