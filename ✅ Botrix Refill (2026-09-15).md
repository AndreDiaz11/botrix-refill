...
## Estado
Funcional: sí | Beta: no (`v1.4.0`) | Última revisión: segunda vuelta de correcciones (15/09/2026) tras probar v1.3.0 en real — contenido que se estiraba al maximizar (ahora con ancho máximo centrado), scroll que cortaba el último grupo, popup de ayuda para sacar el Session-kid, e instancia única reforzada (trae la ventana al frente; **pendiente de que el usuario cierre las instancias viejas que quedaron abiertas de antes del primer fix**, ese candado no las alcanza retroactivamente). Diseño del header todavía sin confirmación visual del usuario — no hay forma de tomar captura de una ventana nativa desde acá. `dotnet build` verificado sin errores.
...
## Versión
1.4.0 — segunda vuelta de correcciones tras feedback del usuario probando v1.3.0 en real: header en una sola línea con ancho máximo (ya no se estira al maximizar), fix de scroll cortado al final de la lista, popup de ayuda para sacar el Session-kid, e instancia única reforzada (ahora trae la ventana existente al frente en vez de solo avisar).

## Cambios
1. (15/09/2026) 4 correcciones más, tras feedback del usuario probando v1.3.0 en real:
